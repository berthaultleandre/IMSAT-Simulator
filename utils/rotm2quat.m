function quat = rotm2quat(rotm)
    ...
    if (tr > epsilon) % if tr > 0:
            % scalar part:
            quat(4,1) = 0.5*sqrt(tr + 1);
            s_inv = 1/(quat(4,1)*4);
            % vector part:
            quat(1,1) = (rotm(3,2) - rotm(2,3))*s_inv;
            quat(2,1) = (rotm(1,3) - rotm(3,1))*s_inv;
        ...
        if ( (rotm(1,1) > rotm(2,2)) && (rotm(1,1) > rotm(3,3)) )
            quat(1,1) = 0.5*sqrt(rotm(1,1) - rotm(2,2) - rotm(3,3) + 1);
            s_inv = 1/(quat(1,1)*4);

            quat(4,1) = (rotm(3,2) + rotm(2,3))*s_inv;
            quat(2,1) = (rotm(2,1) + rotm(1,2))*s_inv;
            quat(3,1) = (rotm(3,1) + rotm(1,3))*s_inv;
        elseif (rotm(2,2) > rotm(3,3))
            quat(2,1) = 0.5*sqrt(rotm(2,2) - rotm(3,3) - rotm(1,1) + 1);
            s_inv = 1/(quat(2,1)*4);

            quat(4,1) = (rotm(1,3) - rotm(3,1))*s_inv;
            quat(1,1) = (rotm(2,1) + rotm(1,2))*s_inv;
            quat(3,1) = (rotm(3,2) + rotm(2,3))*s_inv;
        else
            quat(3,1) = 0.5*sqrt(rotm(3,3) - rotm(1,1) - rotm(2,2) + 1);
            s_inv = 1/(quat(3,1)*4);

            quat(4,1) = (rotm(2,1) - rotm(1,2))*s_inv;
            quat(1,1) = (rotm(3,1) + rotm(1,3))*s_inv;
